...
func BuildEnrichedVulnerability(cveID string) (models.EnrichedVulnerability, error) {
	...
	enriched.PriorityScore = scoring.CalculatePriority(
		enriched.CVSSScore,
		enriched.EPSSScore,
		enriched.KnownExploited,
	)
	cache.EnrichmentCache[cveID] = enriched

	return enriched, nil
}
func BuildMultipleEnrichedVulnerabilities(cveIDs []string) ([]models.EnrichedVulnerability, error) {

	var results []models.EnrichedVulnerability

	for _, cveID := range cveIDs {

		enriched, err := BuildEnrichedVulnerability(cveID)
		if err != nil {
			return nil, err
		}
